    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(fornecedoresRef, (snapshot) => {
          if (snapshot.exists()) {
            ...
            const fornecedoresData: Fornecedores[] = Object.entries(data).map(([key, value]: [string, any]) => ({
              id: key,
      ...
      const handleDelete = async (id: string) => {
        try {
          const fornecedoresRef = ref(database, `Dados/${id}`);
          await remove(fornecedoresRef);
      ...
      const filteredFornecedores = fornecedores.filter(fornecedor => {
        ...
        const matchesTipo = tipoFiltro
          ? fornecedor.nomeFornecedor.toLowerCase() === tipoFiltro.toLowerCase()
          : true;
      ...
      const perPage = 15;
      const paginatedProdutos = paginate(filteredFornecedores, currentPage, perPage);
      ...
      <div className="grid lg:grid-cols-6 gap-4">
        <div className="lg:col-span-1 flex items-center">
          ...
          >
            ...
            <option value="umMes">Guarapuava</option>
            <option value="tresMeses">Curitiba</option>
            <option value="seisMeses">Outra Cidade</option>
          </select>
        </div>
